  ...
  const createNewProject = async () => {
    try {
      const response = await http('/projects', {
        ...
        data: {
          name: boardTitle,
          backgroundColor: stringAvatar(boardTitle).sx.bgcolor,
        },
      })
      ...
      if (response.status === 201) {
        ...
        closeModal()
        dispatch({
          type: 'auth/addProject',
          payload: _id,
        })
        dispatch({
          type: 'project/addProject',
          payload: { ...rest, backgroundColor, name, type: 'owned' },
        })
        navigate('/project', { state: { projectId: _id, projectTitle: name } })
      }
    } catch (error) {
